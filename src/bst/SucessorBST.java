...
public class SucessorBST {
	public static void main(String[] args) {
		...
		SucessorBST bst = new SucessorBST();
		String[] entrada = sc.nextLine().split(" ");
		for(String i: entrada) {
			bst.insert(Integer.parseInt(i));
		}

		bst.sucessor(Integer.parseInt(sc.nextLine()));
		sc.close();
	}
		...
		if (n.isLeaf()) {
			...
			if ((n.getLeft() == null || n.getRight() == null) && n != this.root) {
				...
				if (n.getLeft() != null) {
					...
					ArrayList<Integer> a = new ArrayList<>();
					Node elemento = minimum(n.getRight(),a);
					int data = elemento.getData();
					remove(elemento);
					n.setData(data);
				}
	...
	public Node minimum() {
		ArrayList<Integer> a = new ArrayList<>();
		return minimum(this.root,a);
	}

	private Node minimum(Node node, ArrayList<Integer> a) {
		a.add(node.getData());
		if (node.getLeft() == null) {
			return node;
		} else {
			return minimum(node.getLeft(),a);
		}
	}
	...
	private Node sucessor(Node node, ArrayList<Integer> a) {
		if (node.getRight() != null) {
			return minimum(node.getRight(),a);
		} else {
			Node auxA = node;
			Node auxB = node.getParent();
	...
	class Node {
		...
		private SucessorBST getOuterType() {
			return SucessorBST.this;
		}
