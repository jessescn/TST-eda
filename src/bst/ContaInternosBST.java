...
public class ContaInternosBST {
	public static void main(String[] args) {
		Scanner sc = new Scanner(System.in);
		ContaInternosBST bst = new ContaInternosBST();
		String[] entrada = sc.nextLine().split(" ");
		for (String i : entrada) {
			bst.insert(Integer.parseInt(i));
		}
		System.out.println(bst.contador());
		sc.close();
	}
	...
	private int contador(Node node) {
		int result = 0;

		if (node != null) {
			if (node.isLeaf()) {
				result = contador(node.getLeft()) + contador(node.getRight());
			} else {
				result = 1 + contador(node.getLeft()) + contador(node.getRight());
			}
		}
		return result;
	}
	...
	class Node {
		...
		private ContaInternosBST getOuterType() {
			return ContaInternosBST.this;
		}
